% Code to analyze cam rotation and resulting plunger stroke
...
%%%% Define physical cam parameters
...
%%%% Calculate plunger velocity from angular position/velocity
denom = (lcam^2-dcam^2*sin(theta).^2).^(1/2);
num = dcam*sin(theta).*cos(theta);
Hdot = -dcam*thetadot.*(sin(theta) + num./denom);
for ii=1:n
    H(ii) = sum(Hdot(1:ii))*dt;
end
H = H+lcam-dcam;


figure(1)
plot(time,Hdot,'linewidth',1.5)
xlabel('Time (s)','fontname','times','fontsize',16)
ylabel('Plunger Velocity (m/s)','fontname','times','fontsize',16)
title('Constant \omega','fontname','times','fontsize',16)

figure(2)
plot(time,H,'linewidth',1.5)
xlabel('Time (s)','fontname','times','fontsize',16)
ylabel('Plunger Position (m)','fontname','times','fontsize',16)
title('Constant \omega','fontname','times','fontsize',16)
...
[tout,ThetaSim] = ode45(@CrankFunc,time,theta0,[],params,HM);
